...
// === SKU added to cart and order details pages and email ===

add_filter( 'woocommerce_get_item_data', 'pf_add_sku_to_cart_item_data', 10, 2 );
function pf_add_sku_to_cart_item_data( $item_data, $cart_item ) {
...
add_action( 'woocommerce_order_item_meta_end', 'pf_show_sku_everywhere', 10, 4 );
function pf_show_sku_everywhere( $item_id, $item, $order, $plain_text ) {
    $product = $item->get_product();
    if ( $product && $product->get_sku() ) {
        echo '<br><small><strong>SKU:</strong> ' . esc_html( $product->get_sku() ) . '</small>';
    }
}

/**
 * Ensure WooCommerce variation filtering works on single product pages
 * even if the theme/builder moved the selects or scripts were deferred.
 * Location: /wp-content/themes/divi-child/functions.php (bottom of file)
 */
add_action( 'wp_enqueue_scripts', function () {
    // Only run this fix on single product pages
    if ( ! function_exists('is_product') || ! is_product() ) return;

    // Make sure WooCommerce's variation script is present
    // (If an optimizer had deferred it, this enqueues it properly)
    wp_enqueue_script( 'wc-add-to-cart-variation' );

    // Re-initialize variation behavior after the page loads
    wp_add_inline_script( 'wc-add-to-cart-variation', <<<JS
    (function($){
      $(function(){
        // Find the WooCommerce variations form
        var \$forms = $('.variations_form');

        // If none found, nothing to do (markup may be missing/moved)
        if (!\$forms.length) return;

        \$forms.each(function(){
          var \$f = $(this);

          // Safety: unbind any stale handlers (prevents double-binding)
          \$f.off('found_variation hide_variation reset_data');

          // Bind Woo's default variation logic
          \$f.wc_variation_form();

          // Force a refresh so options are filtered immediately
...
/**
 * STEP 1: Always print all variation data inline on single product pages.
 * Why: Ensures Woo has the full variation JSON on the page (no AJAX needed),
 * which prevents “impossible” option combos from slipping through.
 */
add_filter( 'woocommerce_ajax_variation_threshold', function( $qty, $product ) {
    return 9999; // A very high number so Woo embeds all variations as JSON
}, 10, 2 );