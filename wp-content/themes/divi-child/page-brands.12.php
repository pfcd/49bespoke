...
<div class="brand-directory-wrapper">
    ...
    <div class="brand-grid">
        ...
        $brands = array_filter($all_brands, function($brand) {
            return get_term_meta($brand->term_id, 'show_in_brand_partners', true) === 'yes';
        });


        // Log the query results
        error_log('Brands Query: ' . print_r($brands, true));

        if (!empty($brands) && !is_wp_error($brands)) {
            error_log('Found ' . count($brands) . ' brands with show_in_brand_partners = yes');
            foreach ($brands as $brand) {
                ...
                $show_in_partners = get_term_meta($brand->term_id, 'show_in_brand_partners', true);
                
                // Log each brand's details
                error_log('Brand: ' . $brand->name . ' | show_in_brand_partners: ' . $show_in_partners . ' | Logo URL: ' . $logo_url);

                // Ensure minimum length of 50 characters for description
                $min_length = 50;
            ...
            error_log('No brands found or query error: ' . (is_wp_error($brands) ? $brands->get_error_message() : 'No brands matched criteria'));
...
foreach ($all_brands as $brand) {
    error_log("Brand: {$brand->name} | show_in_brand_partners = " . get_term_meta($brand->term_id, 'show_in_brand_partners', true));
}
