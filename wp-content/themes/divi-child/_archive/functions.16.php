...
function brand_grid_shortcode() {
    ...
    if (is_wp_error($all_brands)) {
        error_log('Brand Grid Shortcode - get_terms Error: ' . $all_brands->get_error_message());
    } elseif (empty($all_brands)) {
        error_log('Brand Grid Shortcode - No brands found for yith_product_brand');
    } else {
        error_log('Brand Grid Shortcode - Found ' . count($all_brands) . ' brands');
        foreach ($all_brands as $brand) {
            $meta_value = get_term_meta($brand->term_id, 'show_in_brand_partners', true);
            error_log("Brand Grid Shortcode - Brand {$brand->name} (ID: {$brand->term_id}) - show_in_brand_partners: '$meta_value'");
        }
    }
