...
function user_manuals_list_shortcode() {
    ...
    // If no brand term found, bail
    if (!isset($pfc_current_brand_term) || !is_object($pfc_current_brand_term)) {
        return '<p>No brand context found.</p>';
    }

    // Pull repeater field from brand term
    $manuals = get_field('user_manuals', $pfc_current_brand_term);

    if (empty($manuals)) {
        return '<p>No user manuals found for this brand.</p>';
    }
    ...
    foreach ($manuals as $manual) {
        if (!empty($manual['file'])) {
            $file = $manual['file'];
            echo '<li><a href="' . esc_url($file['url']) . '" target="_blank">' . esc_html($file['title']) . '</a></li>';
        }
    }
    echo '</ul>';
