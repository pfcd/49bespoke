...
function brand_grid_shortcode() {
    // Step 1: Get all brand terms
    $all_brands = get_terms(array(
        'taxonomy' => 'yith_product_brand',
        'hide_empty' => false
    ));

    // Step 2: Filter brands based on the custom field "show_in_brand_partners"
    $brands = array_filter($all_brands, function($brand) {
        return get_term_meta($brand->term_id, 'show_in_brand_partners', true) === 'yes';
    });

    // Step 3: If no brands, return a message
    if (empty($brands)) return '<p>No brand partners found.</p>';
