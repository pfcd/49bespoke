<?php
// Licensed in this project under GPLv3+; see ../../license.txt for details.
// This file was modified by Jonathan Hall, Dominika Rauk, and/or others. Last modified 2020-06-10
...
if ( ! class_exists( 'TGM_Plugin_Activation' ) ) {
	...
	class TGM_Plugin_Activation {
		...
		public function admin_init() {
			...
			// phpcs:ignore WordPress.Security.NonceVerification -- read-only operation, no CSRF issue
		...
		protected function do_plugin_install() {
			...
			// phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized, ET.Sniffs.ValidatedSanitizedInput.InputNotSanitized -- using our own sanitization function
		...
		public function notices() {
			...
			if ( ! empty( $message ) || $total_required_action_count > 0 ) {
                ...
                $line_template = '<span class="tgmpa-span-block">%s</span>' . "\n";
		...
		protected function is_tgmpa_page() {
			// phpcs:ignore WordPress.Security.NonceVerification -- no CSRF danger with this simple check
...
if ( ! class_exists( 'TGMPA_List_Table' ) ) {
	...
	class TGMPA_List_Table extends WP_List_Table {
		...
		public function __construct() {
			...
			// phpcs:disable WordPress.Security.NonceVerification -- setting the view context is not a CSRF vulnerable operation
		...
		public function column_version( $item ) {
			...
			if ( $this->tgmpa->is_plugin_installed( $item['slug'] ) ) {
				...
				$output[] = sprintf(
                    '<p><span class="tgmpa-span-col" style="%1$s">%2$s</span>' . __( 'Installed version:', 'tgmpa' ) . '</p>',
					$color,
					$installed
				);
			}
			...
			if ( ! empty( $item['minimum_version'] ) ) {
				$output[] = sprintf(
                    '<p><span class="tgmpa-span-col">%1$s</span>' . __( 'Minimum required version:', 'tgmpa' ) . '</p>',
			...
			if ( ! empty( $item['available_version'] ) ) {
				...
				$output[] = sprintf(
                    '<p><span class="tgmpa-span-col" style="%1$s">%2$s</span>' . __( 'Available version:', 'tgmpa' ) . '</p>',
					$color,
		...
		public function process_bulk_actions() {
			...
			if ( 'tgmpa-bulk-install' === $this->current_action() || 'tgmpa-bulk-update' === $this->current_action() ) {
				...
				if ( is_array( $_POST['plugin'] ) ) {
					// phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized, ET.Sniffs.ValidatedSanitizedInput.InputNotSanitized -- to be sanitized after "if" block
					$plugins_to_install = wp_unslash( (array) $_POST['plugin'] );
                ...
                '<h2 class="tgmpa-heading-h2">', esc_html( get_admin_page_title() ), '</h2>
			...
			if ( 'tgmpa-bulk-activate' === $this->current_action() ) {
				...
				if ( isset( $_POST['plugin'] ) ) {
					// phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized, ET.Sniffs.ValidatedSanitizedInput.InputNotSanitized -- will be sanitized on the next line
					$plugins = array_map( 'urldecode', wp_unslash( (array) $_POST['plugin'] ) );
...
if ( ! function_exists( 'tgmpa_load_bulk_installer' ) ) {
	...
	function tgmpa_load_bulk_installer() {
		...
		// phpcs:ignore WordPress.Security.NonceVerification -- checking the page variable is not a CSRF risk
