...
if (defined('AGS_THEME_ADDONS_URL')) {

    class AGS_Theme_Addons {

        static function outputList() {

            load_theme_textdomain('ags-divi-ecommerce-pro-addons', __DIR__ . '/languages');

            $addonsJson = wp_remote_retrieve_body(
                wp_remote_get(
                    add_query_arg('locale', get_locale(), AGS_THEME_ADDONS_URL)
                )
            );
            ...
            if (empty($addons)) {
                echo(
                    ...
                    . esc_html__('We are unable to load the addons for this theme right now. Please try again later!', 'ags-divi-ecommerce-pro-addons')
                    . '</p>'
                );
                return;
            }
                ...
                <div class="ags-settings-addon-info">
                    <img src="<?php echo esc_url(
                        empty($addon->thumbnail)
                            ? get_stylesheet_directory_uri() . '/admin/addons/images/placeholder.jpg'
                            : $addon->thumbnail
            ...
            switch ($addon->type) {
                ...
                case 'repo':
                    ...
                    if (empty($installed_plugin)) {
                        $label = __('Install', 'ags-divi-ecommerce-pro-addons');
                        if (current_user_can('install_plugins')) {
                            $url = wp_nonce_url(self_admin_url('update.php?action=install-plugin&plugin=' . $addon->target), 'install-plugin_' . $addon->target);
                            $onclick_escaped = '';
                        } else {
                            $url = '#';
                            $onclick_escaped = ' onclick="alert(\'' . esc_js(__('You do not have sufficient permissions to install plugins. If you think this message is in error, go to Plugins > Add New and search for the plugin name.', 'ags-divi-ecommerce-pro-addons')) . '\');return false;"';
                        }
                    } else {
                        $label = __('Already Installed', 'ags-divi-ecommerce-pro-addons');
    ...
    add_action('admin_menu', function() {
        add_menu_page( 'Addons Test', 'Addons Test', 'manage_options', 'ags-divi-ecommerce-pro-addons', [ 'AGS_Theme_Addons', 'outputList' ] );
    });
    */
