...
class Dsm_Supreme_Modules_Pro_For_Divi {
	...
	public function dsm_add_popup_modules_setting( $fields_unprocessed ) {
		...
		$fields['dsm_modules_popup_removal_delay']   = array(
			'label'            => esc_html__( 'Removal Delay', 'dsm-supreme-modules-pro-for-divi' ),
			'type'             => 'range',
			'option_category'  => 'layout',
			'tab_slug'         => 'custom_css',
			'toggle_slug'      => 'visibility',
	...
	public function output_modules_popup( $output, $render_slug, $module ) {
		if ( $this->dsm_load_divi_modules() !== $render_slug ) {
			...
			if ( isset( $module->props['dsm_modules_popup'] ) && 'on' === $module->props['dsm_modules_popup'] ) {
				$dsm_modules_popup_removal_delay          = isset( $module->props['dsm_modules_popup_removal_delay'] ) ? $module->props['dsm_modules_popup_removal_delay'] : esc_attr( '500' );
				$dsm_modules_popup_animation_in           = isset( $module->props['dsm_modules_popup_animation_in'] ) ? $module->props['dsm_modules_popup_animation_in'] : esc_attr( 'fadeIn' );
				$dsm_modules_popup_animation_out          = isset( $module->props['dsm_modules_popup_animation_out'] ) ? $module->props['dsm_modules_popup_animation_out'] : esc_attr( 'fadeOut' );
				$dsm_modules_popup_trigger                = isset( $module->props['dsm_modules_popup_trigger'] ) ? $module->props['dsm_modules_popup_trigger'] : esc_attr( 'module' );
				...
				$dsm_modules_popup_position               = isset( $module->props['dsm_modules_popup_position'] ) ? $module->props['dsm_modules_popup_position'] : esc_attr( 'center' );
				...
				$dsm_modules_popup_position_type          = isset( $module->props['dsm_modules_popup_position_type'] ) ? $module->props['dsm_modules_popup_position_type'] : esc_attr( 'absolute' );
				$dsm_modules_popup_button                 = isset( $module->props['dsm_modules_popup_button'] ) ? $module->props['dsm_modules_popup_button'] : 'on';
				$dsm_modules_popup_close_button_placement = isset( $module->props['dsm_modules_popup_close_button_placement'] ) ? $module->props['dsm_modules_popup_close_button_placement'] : 'true';
				...
				$dsm_modules_popup_width        = isset( $module->props['dsm_modules_popup_width'] ) ? $module->props['dsm_modules_popup_width'] : esc_attr( '680px' );
				...
				$dsm_modules_close_font_icon_size        = isset( $module->props['dsm_modules_close_font_icon_size'] ) ? $module->props['dsm_modules_close_font_icon_size'] : esc_attr( '24px' );
				...
				$output = str_replace( 'class="dsm_pricelist_child ', $data . ' class="dsm_pricelist_child dsm-popup-' . $dsm_modules_popup_trigger . $dsm_modules_popup_check_trigger . ' ', $output );
				...
				// To output before body tag.
				add_action(
					'wp_footer',
					function () use ( $dsm_custom_popup ) {
						// Divi already escaped - Don't see a reason why using kses is needed?.
						//phpcs:ignore
						echo et_core_esc_previously( $dsm_custom_popup );
					}
				);
			...
			foreach ( $_attachments as $key => $val ) {
				...
				foreach ( $current_categories as $category ) {
					$attachment_categories[] = array(
						...
						'id'   => isset( $category->term_id ) ? $category->term_id : null,
