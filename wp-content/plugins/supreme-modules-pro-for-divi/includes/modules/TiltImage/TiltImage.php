...
class DSM_Tilt_Image extends ET_Builder_Module {
	...
	public function render( $attrs, $content, $render_slug ) {
		...
		$is_overlay_applied = '' !== $dsm_overlay_title || '' !== $content || 'on' === $use_overlay || ( 'on' === $show_in_lightbox || ( 'off' === $show_in_lightbox && '' !== $url ) ) ? 'on' : 'off';
		...
		if ( '' !== $dsm_overlay_title ) {

			$title_attr = array(
				'class'                    => 'dsm_tilt_overlay_title',
				'data-overlay-title-hover' => $dsm_overlay_title__hover,
			);

			if ( $dsm_overlay_title_hover_enabled ) {
				unset( $title_attr['data-overlay-title-hover'] );
			}

			$dsm_overlay_title = $multi_view->render_element(
				array(
					'tag'     => et_pb_process_header_level( $header_level, 'h4' ),
					'content' => '<span>{{dsm_overlay_title}}</span>',
					'attrs'   => $title_attr,
				)
			);
		}
		...
		if ( '' !== $this->content ) {
			$overlay_content = $multi_view->render_element(
				array(
					'tag'     => 'div',
					'content' => '{{content}}',
					'attrs'   => array(
						'class' => 'dsm_tilt_overlay_content',
					),
				)
			);
		}
		...
		if ( 'on' === $dsm_use_parallax ) {
			ET_Builder_Element::set_style(
				$render_slug,
				array(
					'selector'    => '%%order_class%% .dsm-tilt-image-wrapper,%%order_class%% .dsm-tilt-image-wrapper .et_pb_image_wrap',
