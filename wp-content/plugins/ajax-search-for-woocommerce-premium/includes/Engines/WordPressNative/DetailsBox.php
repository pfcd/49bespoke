...
class DetailsBox {
	...
	private function getPostTypeDetails__premium_only( $postID ) {
		...
		$vars = array(
			'ID'          => $postID,
			'title'       => $post->getTitle(),
			'link'        => apply_filters( "dgwt/wcas/suggestion_details/{$postType}/url", $post->getPermalink(), $post ),
		...
		ob_start();
		Helpers::loadTemplate( 'details-panel/' . $postType . '.php', $vars, 'details-panel/post-type.php' );
		$html = ob_get_clean();
