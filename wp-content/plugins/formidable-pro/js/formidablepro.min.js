...
unprocessedPendingLookups.slice(start,end);currentBatch.forEach(function(pendingLookup){pendingLookup.pending=false;allFormIds[pendingLookup.childFieldArgs.formId]=pendingLookup.childFieldArgs.formId});allFormIds=Object.values(allFormIds);postToAjaxUrl(getFormById(formId),{action:"frm_replace_lookup_field_options_arr",postData:currentBatch.map(function(pendingLookup){return pendingLookup.childFieldArgs}),nonce:frm_js.nonce},function(newOptionsByFieldId){allFormIds.forEach(enableFormAfterLookup);Object.entries(newOptionsByFieldId).forEach(function(entry){var key=
entry[0];var newOptions=entry[1];unprocessedPendingLookups.forEach(function(pendingLookup){if(pendingLookup.childFieldArgs.unique===parseInt(key))pendingLookup.callback(newOptions)})})},false,{dataType:"json"})}}function getUnprocessedPendingLookups(){return pendingLookupFieldAjax.filter(function(pendingLookup){return pendingLookup.pending})}function getRepeatingFieldDivOnCurrentPage(childFieldArgs,parentRepeatArgs){var childFieldDivs=[],selector="frm_field_"+childFieldArgs.fieldId+"-";selector+=
...
formId+"_container");if(form!==null)form=form.closest("form")}return form}function enableLookup(childSelect,isReadOnly){if(isReadOnly===false)childSelect.disabled=false;childSelect.className=childSelect.className.replace(" frm_loading_lookup","")}function isMultiSelect(element){return element.tagName.toLowerCase()==="select"&&element.multiple}function getSelectedOptions(element){if(isSlimSelect(element))return element.slim.getSelected();if(!isMultiSelect(element))return element.value;var i,option,
selectedOptions=[];for(i=0;i<element.options.length;i++){option=element.options[i];if(option.selected)selectedOptions.push(option.value)}return selectedOptions}function setSelectedOptions(element,values){var option;if(!isMultiSelect(element)){element.value=values;return}Array.from(element.options).forEach(function(option){option.selected=false});values.forEach(function(value){option=element.querySelector('option[value="'+value+'"]');if(option)option.selected=true})}function replaceSelectLookupFieldOptions(fieldArgs,
...
1)calcField=filterCalcField(calcField,field.thisFieldId);return calcField}function filterCalcField($calcField,thisFieldId){return $calcField.filter(function(){var target="OPTION"===this.nodeName?this.closest("select"):this;return target&&target.name&&target.name.indexOf(thisFieldId)!==-1})}function getOffScreenFieldForName(field){var nameParts,input;nameParts=[];document.querySelectorAll(field.thisFieldCall).forEach(function(input){nameParts.push(input.value)});input=document.createElement("input");
input.value=nameParts.join(" ");return jQuery(input)}function getDateFieldValue(dateFormat,thisVal){var d=0;if(!thisVal);else if(typeof jQuery.datepicker==="undefined"){var splitAt="-";if(dateFormat.indexOf("/")>-1)splitAt="/";var year="",month="",day="",formatPieces=dateFormat.split(splitAt),datePieces=thisVal.split(splitAt);for(var i=0;i<formatPieces.length;i++)if(formatPieces[i]==="y"){var currentYear=(new Date).getFullYear()+15;var currentYearPlusFifteen=currentYear.toString().substr(2,2);if(datePieces[i]>
...
"function")return;phoneInputs.forEach(function(phoneInput){var intlTelInputInstance=window.intlTelInput(phoneInput,{initialCountry:"auto",formatOnDisplay:false,geoIpLookup:function(callback){if("function"===typeof window.fetch)fetch("https://ipapi.co/json").then(function(res){return res.json()}).then(function(data){callback(data.country_code)}).catch(function(){callback("us")})}});phoneInput.addEventListener("blur",function(){phoneInput.value=intlTelInputInstance.getNumber()});intlPhoneInputs[phoneInput.id]=
intlTelInputInstance})}function maybeUpdateFormsOverflowX(){document.querySelectorAll(".frm-show-form").forEach(function(slideinForm){slideinForm.style["overflow-x"]="visible"})}function initRangeInput(rangeInputs){rangeInputs.forEach(function(rangeInput){updateRangeInputBackground(rangeInput);rangeInput.addEventListener("input",function(){updateRangeInputBackground(rangeInput)});rangeInput.addEventListener("onchange",function(){updateRangeInputBackground(rangeInput)})});function updateRangeInputBackground(rangeInput){var sliderValuePercent=
(rangeInput.value-rangeInput.min)/(rangeInput.max-rangeInput.min)*100;rangeInput.style.background="linear-gradient(to right, var(--slider-color) 0%, var(--slider-color) "+sliderValuePercent+"%, var(--slider-bar-color) "+sliderValuePercent+"% 100%)"}}return{init:function(){maybeUpdateFormsOverflowX();maybeAddIntlTelInput(document.querySelectorAll(".frm-intl-tel-input"));initRangeInput(document.querySelectorAll(".with_frm_style input[type=range]"));addEventListener("pageshow",maybeTriggerCalc);jQuery(document).on("frmFormComplete",
