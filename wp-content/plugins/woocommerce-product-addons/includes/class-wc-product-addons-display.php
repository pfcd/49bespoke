...
class WC_Product_Addons_Display {
	...
	public function styles() {
		...
		if (
			...
			wp_enqueue_script( 'jquery-tiptip', WC()->plugin_url() . '/assets/js/jquery-tiptip/jquery.tipTip.min.js', array( 'jquery', 'dompurify' ), WC_VERSION, true );
	...
	public function addon_scripts() {
		...
		$params = array(
			...
			'i18n_validation_decimal_separator'        => sprintf( __( 'Please enter a price with one monetary decimal point (%1$s) without thousand separators.', 'woocommerce-product-addons' ), '%c' ),
	...
	public function display( $post_id = false, $prefix = false ) {
		...
		if ( is_array( $product_addons ) && count( $product_addons ) > 0 ) {
			...
			foreach ( $product_addons as $addon ) {
				...
				echo WC_Product_Addons_Html_Generator::get_addon_html( $addon, $value ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	...
	protected function get_cart_item_from_query(): ?array {
		...
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended  -- Skipping nonce verification since this doesn't result in any specific action.
