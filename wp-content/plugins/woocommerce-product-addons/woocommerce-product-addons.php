...
function woocommerce_product_addons_init() {
	load_plugin_textdomain( 'woocommerce-product-addons', false, plugin_basename( __DIR__ ) . '/languages' );
	...
	if ( ! function_exists( 'phpversion' ) || version_compare( phpversion(), '7.4.0', '<' ) ) {
		...
		$notice = sprintf(
			__(
				'WooCommerce Product Add-Ons requires at least PHP <strong>%1$s</strong>. Learn <a href="%2$s">how to update PHP</a>.',
	...
	if ( ! function_exists( 'WC' ) || version_compare( WC()->version, '8.2.0' ) < 0 ) {
		$notice = __( 'WooCommerce Product Add-Ons requires at least WooCommerce version <strong>%1$s</strong>. %2$s', 'woocommerce-product-addons' );
		if ( ! function_exists( 'WC' ) ) {
			$notice = sprintf( $notice, '8.2.0', __( 'Please install and activate WooCommerce.', 'woocommerce-product-addons' ) );
		} else {
			$notice = sprintf( $notice, '8.2.0', __( 'Please update WooCommerce.', 'woocommerce-product-addons' ) );
		}
		require_once __DIR__ . '/includes/admin/class-wc-product-addons-admin-notices.php';
		WC_PAO_Admin_Notices::add_notice( $notice, 'error' );
	...
	if ( ! class_exists( 'WC_Product_Addons' ) ) :
		...
		define( 'WC_PRODUCT_ADDONS_VERSION', '7.9.2' ); // WRCS: DEFINED_VERSION.
		...
		class WC_Product_Addons {
			...
			public function init() {
				...
				// Tracker.
				require_once __DIR__ . '/includes/class-wc-product-addons-tracker.php';
				require_once __DIR__ . '/includes/class-wc-product-addons-tracks.php';
			...
			public function plugin_row_meta( $links, $file ) {
				...
				if ( $file === plugin_basename( __FILE__ ) ) {
					$plugin_links = array(
						'<a href="https://woocommerce.com/document/product-add-ons/">' . esc_html__( 'Documentation', 'woocommerce-product-addons' ) . '</a>',
			...
			public function notices() {
				...
				if ( $show_activate_notice ) {
					...
					$onboarding_message           = sprintf(
						...
						$this->get_resource_url( 'global-addons' ),
						$this->get_resource_url( 'per-product-addons' )
					);
