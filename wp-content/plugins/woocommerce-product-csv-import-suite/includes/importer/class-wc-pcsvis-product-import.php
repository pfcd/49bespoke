...
class WC_PCSVIS_Product_Import extends WP_Importer {
		...
		switch ( $step ) {
			...
			case 2 :
				...
				if ( $this->id )
					...
					$file = WP_CONTENT_DIR . $this->file_url; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
					...
					<script type="text/javascript">
						jQuery(document).ready(function($) {
							...
							if ( version_compare( PHP_VERSION, '8.1', '<' ) ) {
								@ini_set( 'auto_detect_line_endings', true ); // phpcs:ignore WordPress.PHP.NoSilencedErrors.Discouraged, PHPCompatibility.IniDirectives.RemovedIniDirectives
							}
			...
			case 3 :
				...
				echo "<!--WC_START-->";
				echo wc_esc_json( wp_json_encode( $results ), true ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				echo "<!--WC_END-->";
				exit;
			break;
			case 4 :
	...
	protected function handle_initial_upload_file() {
		...
		if ( isset( $file['error'] ) ) {
			...
			throw new Exception( sprintf( esc_html__( 'Sorry, there has been an error: %s.', 'woocommerce-product-csv-import-suite' ), $file['error'] ) ); // phpcs:ignore
		}
	...
	protected function handle_initial_path_file_check() {
		...
		$this->perform_file_path_checks( WP_CONTENT_DIR . $file_url ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	...
	private function perform_file_path_checks( string $filepath ) {
		if ( 'phar' === strtolower( (string) wp_parse_url( $filepath, PHP_URL_SCHEME ) ) ){ // phpcs:ignore
		...
		if ( ! $this->is_within_content_dir( $filepath ) ) {	/* translators: placeholder is base directory (WP_CONTENT_DIR) */
			throw new Exception( sprintf( esc_html__( 'Sorry, there has been an error: filepath must exist inside %s.', 'woocommerce-product-csv-import-suite' ), WP_CONTENT_DIR ) ); // phpcs:ignore
		}

		if ( ! file_exists( $filepath ) ) {
			/* translators: placeholder is file path */
			throw new Exception( sprintf( esc_html__( 'Sorry, there has been an error: %s does not exist.', 'woocommerce-product-csv-import-suite' ), $filepath ) ); // phpcs:ignore
		}

		if ( ! $this->is_acceptable_csv_file( $filepath ) ) {
			$mime_types = implode( ', ', $this->get_acceptable_csv_mime_types() ); // phpcs:ignore

			/* translators: placeholder is comma-separated of accepted mime-types for import (e.g. 'text/csv') */
			throw new Exception( sprintf( esc_html__( 'File must have .csv extension with acceptable mime types (%s)', 'woocommerce-product-csv-import-suite' ), $mime_types ) ); // phpcs:ignore
		}
	}
	...
	private function is_within_content_dir( string $filepath ): bool {
		return 0 === strpos( wp_normalize_path( realpath( $filepath ) ), wp_normalize_path( trailingslashit( realpath( WP_CONTENT_DIR ) ) ) ); // phpcs:ignore
	}
	...
	public function process_product( $post ) {
		...
		$wpdb->query( $wpdb->prepare( "DELETE FROM $wpdb->postmeta WHERE meta_key IN ( '" . implode( "','", array_map( 'esc_sql', array_keys( $insert_meta_data ) ) ) . "' ) and post_id = %d", $post_id ) ); // phpcs:ignore
		...
		$wpdb->query( "INSERT INTO $wpdb->postmeta ( post_id, meta_key, meta_value ) VALUES " . implode( ',', $meta_values ) ); // phpcs:ignore
		$wpdb->query( 'COMMIT' );
	...
	public function process_attachment( $post, $url, $post_id ) {
		...
		if ( strstr( $url, site_url() ) ) {
			$abs_url 	= str_replace( trailingslashit( site_url() ), trailingslashit( WP_CONTENT_DIR ), $url ); // phpcs:ignore
		...
		if ( ! strstr( $url, 'http' ) ) {
			...
			if ( file_exists( $attachment_file ) ) {
				...
				$attachment_url = str_replace( trailingslashit( WP_CONTENT_DIR ), trailingslashit( site_url() ), $attachment_file ); // phpcs:ignore
				...
				if ( ! $attachment_id ) {
					if ( $info = wp_check_filetype( $attachment_file ) )
						...
						return new WP_Error( 'attachment_processing_error', __( 'Invalid file type', 'woocommerce-product-csv-import-suite' ) );
				...
				return new WP_Error( 'attachment_processing_error', __( 'Local image did not exist!', 'woocommerce-product-csv-import-suite' ) );
			}
	...
	public function fetch_remote_file( $url, $post ) {
		...
		// Upload the file
		$upload = wp_upload_bits( $file_name, '', wp_remote_retrieve_body( $response ) ); // phpcs:ignore
		...
		if ( 0 == $filesize ) {
			@unlink( $upload['file'] ); // phpcs:ignore
			unset( $upload );
