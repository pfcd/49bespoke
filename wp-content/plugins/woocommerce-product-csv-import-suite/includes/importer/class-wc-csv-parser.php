...
class WC_CSV_Parser {
	...
	public function parse_product( $item, $merge_empty_cells = 0 ) {
		...
		if ( $merging ) {
			...
			// translation: %s is the row number.
			...
			elseif ( strstr( $key, 'attribute:' ) ) {
				...
				if ( substr( $attribute_key, 0, 3 ) == 'pa_' ) {
					...
					if ( ! taxonomy_exists( $taxonomy ) ) {
						...
						$exists_in_db = $wpdb->get_var( "SELECT attribute_id FROM " . $wpdb->prefix . "woocommerce_attribute_taxonomies WHERE attribute_name = '" . $nicename . "';" ); // phpcs:ignore
