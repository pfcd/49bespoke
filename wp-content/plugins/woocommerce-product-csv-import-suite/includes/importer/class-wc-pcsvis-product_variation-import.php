...
class WC_PCSVIS_Product_Variation_Import extends WC_PCSVIS_Product_Import {
	...
	public function process_product( $post ) {
		...
		if ( ! $post_parent_exists ) {
			...
			// translators: %d is the parent ID.
		...
		if ( $merging ) {
			...
			if ( is_wp_error( $post_id ) ) {
				...
				$postdata['ID']         = $post_id;
				$postdata['post_title'] = sprintf( __( 'Variation #%s of %s', 'woocommerce-product-csv-import-suite' ), $post_id, get_the_title( $post_parent ) ); // phpcs:ignore
