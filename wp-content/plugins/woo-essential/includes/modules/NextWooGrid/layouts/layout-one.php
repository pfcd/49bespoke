...
$categorie_list      = wc_get_product_category_list( $product_id , ', ', '<li>', '</li>');

$product_rating      = wc_get_rating_html($product->get_average_rating(), $product->get_rating_count());
$dataIcon            = 'data-icon=""';
$dataIconChooseOption = 'data-icon="a"';
...
$product_price_html = 'off' !== $show_price_text ? sprintf('<div class="dnwoo_product_grid_price">%1$s</div>', $product->get_price_html()) : '';

$add_to_cart_icon = $this->_add_to_cart_icon($product_id, $product_type, $permalink, $dataIcon, $dataIconChooseOption);
...
$product_btn_icon = sprintf(
    ...
    "off" !== $show_add_to_cart_icon ? $add_to_cart_icon : '',
    "off" !== $show_wish_list_icon ? dnwoo_add_to_wishlist_button() : '',
    "off" !== $show_add_compare_icon ? dnwoo_product_compare_button() : '',
    "off" !== $show_quickview_icon ? $quickview_icon : ''
);


$product_btn = $this->_add_to_cart($product_id, $product_type, $permalink, $show_add_to_cart, $dnwoo_show_add_to_cart_text, $dnwoo_select_option_text, $dataIconChooseOption, $dataIcon);

// if( 'variable' === $product_type ) {
//     $product_btn = sprintf('<a href="%1$s" class="product_type_variable dnwoo_choose_variable_option">
//         <span class="icon_menu_btn" %3$s></span> %2$s
//     </a>', 
//         $permalink, 
//         $dnwoo_select_option_text,
//         $dataIconChooseOption
//     );
// }else {
//     $product_btn = sprintf(
...
//         'simple' == $product->get_type() ? $permalink : sprintf('?add-to-cart=%1$s', $product_id),
//         $product_id,
//         $product_type,
//         'on'=== $show_add_to_cart ? $dnwoo_show_add_to_cart_text : '',
//         $dataIcon
//     );
// }


$quickview_btn = sprintf(
    '<a href="#" class="dnwoo_product_grid_quick_button dnwoo-quick-btn dnwoo-quickview" data-quickid="%2$s" data-orderclass="%3$s">
    <span class="icon_quickview_btn" %4$s></span> %1$s
    </a>',
    $dnwoo_quick_view_text,
    $product->get_id(),
    $order_class,
    $data_icon_quickview
);
...
$single_products .= sprintf(
    '<div class="dnwoo_product_grid_img">
        <a href="%1$s" class="dnwoo_product_img">
            <img class="img-fluid" src="%2$s" alt="Product Image" >
        </a>
...
$single_products .= sprintf(
    ...
    <%8$s class="dnwoo_product_grid_title"><a href="%3$s">%2$s</a></%8$s>
    %9$s
    %4$s
    ...
    $permalink,
    $product_price_html,
    'on' === $show_add_to_cart ? $product_btn : '',
    "on" === $show_category ? $categorie_list : '',
    'off' !== $show_quick_view_button ? $quickview_btn : '',
    $dnwooe_tag,
    sku_template($sku)
);
