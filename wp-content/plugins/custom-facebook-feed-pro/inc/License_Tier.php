...
abstract class License_Tier
{
	...
	public $item_id_smash; // put item id for the elite tier
	...
	public $license_tier_smash_name; // smash tier name
	...
	public function legacy_tier_features()
	{
		...
		if ($plan_name == $this->license_tier_developer_name || $plan_name == $this->license_tier_smash_name) {
			$tier_features = flatten_array($all_features);
		}
		return $tier_features;
	}
	...
	public function item_id_to_tier_name()
	{
		$item_id_to_tier_name = [
			...
			$this->item_id_smash => $this->license_tier_smash_name,
		];
	...
	public function item_name_to_tier_name()
	{
		...
		if (strpos($this->edd_item_name, 'Smash') !== false) {
			$license_tier_name = $this->license_tier_smash_name;
		}
