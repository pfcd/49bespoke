 ...
 * @since ?
 */
abstract class DIPI_Builder_Module_Settings_Migration {
	 ...
	 * @var string
	 */
	public static $max_version = '4.21';
	...
	public static $migrations = array(
		...
		'4.17'    => 'DipiBackgroundGradientStops',
	);
	...
	public static function get_migrations( $module_version, $module_slug = '' ) {
		
		
		if ( isset( self::$migrations_by_version[ $module_version ] ) && !empty(self::$migrations_by_version[ $module_version ] )) {
		...
		foreach ( self::$migrations as $version => $migration ) {
			...
			if ( is_string( $migration ) ) {
				$migration                    = require_once "{$migration}.php";
	...
	public static function init() {
		$class = 'DIPI_Builder_Module_Settings_Migration';

		add_filter( 'et_pb_module_processed_fields', array( $class, 'maybe_override_processed_fields' ), 30, 2 );
		add_filter( 'et_pb_module_shortcode_attributes', array( $class, 'maybe_override_shortcode_attributes' ), 30, 6 );
		add_filter( 'et_pb_module_content', array( $class, 'maybe_override_content' ), 30, 6 );
		require_once __DIR__ . '/DipiImageSliderSettings.php';
	}
	...
	public static function maybe_override_shortcode_attributes( $attrs, $unprocessed_attrs, $module_slug, $module_address, $content = '', $maybe_global_presets_migration = false ) {
		...
		$migrations                            = self::get_migrations( $attrs['_builder_version'], $module_slug  );
		...
		foreach ( $migrations as $migration ) {
			...
			$classes_requiring_presets_migration = array(
			 	'DIPI_Builder_Module_Settings_Migration_BackgroundGradientStops'
			);
	...
	public static function _should_handle_render( $slug ) {
		...
		$all_module_slugs = null;
		if( !method_exists(ET_Builder_Element::class, 'get_all_module_slugs')){
			return false;
		} else{
