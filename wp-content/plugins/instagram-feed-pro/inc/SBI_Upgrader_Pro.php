...
class SBI_Upgrader_Pro
{
    ...
    {
        add_action('wp_ajax_nopriv_sbi_run_one_click_upgrade', array( 'InstagramFeed\Admin\SBI_Upgrader_Pro', 'install_upgrade' ));
        add_action('wp_ajax_sbi_maybe_upgrade_redirect', array( 'InstagramFeed\Admin\SBI_Upgrader_Pro', 'maybe_upgrade_redirect' ));
        add_action('sb_instagram_twicedaily', array( 'InstagramFeed\Admin\SBI_Upgrader_Pro', 'check_license_upgraded_cron' ));
    }
    ...
    public static function maybe_upgrade_redirect()
    {
        ...
        if (SBI_Upgrader_Pro::is_dev_url(home_url()) ) {
        ...
        if (! is_wp_error($response) ) {
            ...
            if (empty($check_key_response['license_data']) ) {
                wp_send_json_error(
                    array(
                    'message' => esc_html(SBI_Upgrader_Pro::get_error_message($check_key_response)),
                    )
                );
            }
            ...
            if (! empty($check_key_response['license_data']['error']) ) {
                wp_send_json_error(
                    array(
                    'message' => SBI_Upgrader_Pro::get_error_message($check_key_response),
                    )
                );
            }
            ...
            $version      = '1.0';
            $version_info = SBI_Upgrader_Pro::get_version_info($license_data);
            $file         = '';
            ...
            wp_send_json_success(
                array(
                'url' => $url,
                'same_version' => version_compare(SBIVER, $check_key_response['current_version'], '='),
    ...
    public static function install_upgrade()
    {
        ...
        if (! empty($file) ) {
            delete_option('sbi_islicence_upgraded');
            delete_option('sbi_upgraded_info');
    ...
    public static function check_license_upgraded( $current_license_data, $license )
    {
        ...
        if (! is_wp_error($response) ) {
            ...
            if(isset($current_license_data['item_name'], $current_license_data['item_id'], $license_data['item_name'], $license_data['item_id'])
                ...
                update_option('sbi_islicence_upgraded', true);
                update_option('sbi_upgraded_info', $license_data);
            }
    ...
    public static function check_license_upgraded_cron()
    {
        ...
        if(isset($sbi_license_data['success'], $sbi_license_data['license'])
            && $sbi_license_data['success'] === true
            && $sbi_license_data['license'] === 'valid'
        ) {
            SBI_Upgrader_Pro::check_license_upgraded($sbi_license_data, $sbi_license_data['key']);
        }
    }
