...
class Settings {
	...
	/**
	 * Retrieves the design style theme setting.
	 *
	 * @return string The selected theme style or 'default' if not set.
	 */
	public static function get_design_style_theme() {
		$options = self::get_setting_table_styling();
		return ! empty( $options['use_theme'] ) ? $options['use_theme'] : 'default';
	}
	...
	public static function merge_design_defaults_with_options( $defaults, $options ) {
		$theme             = ! empty( $options['use_theme'] ) ? $options['use_theme'] : 'theme';
		$formatted_options = [];

		unset( $options['use_theme'] );
