 ...
 * WPForms functions
 *
 * @package YITH\CatalogMode\Integrations\Forms\WPForms
...
if ( ! function_exists( 'ywctm_wpforms_entry_email_atts' ) ) {
	...
	function ywctm_wpforms_entry_email_atts( $email, $fields, $entry ) {
		...
		if ( isset( $entry['ywctm-product-id'] ) ) {
			...
			$product_id = $entry['ywctm-product-id'];
			$params     = explode( ',', $entry['ywctm-params'] );
			$wpforms    = ywctm_get_localized_form( 'wpforms', $product_id );

			if ( (int) $entry['id'] === (int) $wpforms && apply_filters( 'ywctm_get_vendor_option', get_option( 'ywctm_inquiry_product_permalink' ), $product_id, 'ywctm_inquiry_product_permalink' ) === 'yes' ) {
				...
				if ( 'none' !== wpforms_setting( 'email-template', 'default' ) ) {
					...
					<table align="left" border="0" cellpadding="0" cellspacing="0" width="100%" style="border-top:1px solid #dddddd; display:block;min-width: 100%;border-collapse: collapse;width:100%;">
						...
						<tr>
							<td style="color:#555555;padding-top: 3px;padding-bottom: 20px;"><?php echo wp_kses_post( ywctm_get_product_link( $product_id, $params ) ); ?></td>
						</tr>
						</tbody>
					</table>
					<?php
					...
					$email['message'] .= ywctm_get_product_link( $product_id, $params, false ) . "\r\n\r\n";
				}
			}
		}
