<?php
/**
 * Formidable forms functions
 *
 * @package YITH\CatalogMode\Integrations\Forms\FormidableForms
...
if ( ! function_exists( 'ywctm_formidable_forms_message' ) ) {
	...
	function ywctm_formidable_forms_message( $values ) {
		...
		if ( isset( $values['ywctm-product-id'] ) ) {
			...
			$product_id      = $values['ywctm-product-id'];
			$params          = explode( ',', $values['ywctm-params'] );
			$form_id         = $values['form_id'];
			$field_id        = $values['ywctm-ff-field-id'];
			$formidable_form = ywctm_get_localized_form( 'formidable-forms', $product_id );

			if ( (int) $form_id === (int) $formidable_form && apply_filters( 'ywctm_get_vendor_option', get_option( 'ywctm_inquiry_product_permalink' ), $product_id, 'ywctm_inquiry_product_permalink' ) === 'yes' ) {
				$values['item_meta'][ $field_id ] = ywctm_get_product_link( $product_id, $params );
			}
		}
