<?php
/**
 * Buttons & Labels post type class
 *
...
if ( ! class_exists( 'YWCTM_Button_Label_Post_Type' ) ) {
	...
	class YWCTM_Button_Label_Post_Type {
		...
		public function __construct() {
			...
			add_filter( 'yith_wcmv_catalog_mode_module_capabilities', array( $this, 'get_capabilities' ) );
			add_action( "yith_wcmv_restrict_edit_{$this->post_type}_vendor", array( $this, 'restrict_buttons_edit' ), 10, 2 );
			...
			add_filter( "manage_{$this->post_type}_posts_custom_column", array( $this, 'render_custom_columns' ), 10, 2 );
			add_filter( "views_edit-$this->post_type", array( $this, 'set_views' ) );
			...
			add_filter( 'ywctm_button_editor_fonts', array( $this, 'add_default_fonts' ) );
		 ...
		 * @since   1.15.0
		 */
		public function restrict_buttons_edit( $post, $vendor ) {
			...
			if ( ! $post_author || ! in_array( $post_author, $vendor->get_admins(), true ) ) {
				...
				wp_die( sprintf( esc_html__( 'You do not have permission to edit this button. %1$sClick here to view and edit your button%2$s.', 'yith-woocommerce-catalog-mode' ), '<a href="' . esc_url( "edit.php?post_type=$this->post_type" ) . '">', '</a>' ) );
			}
		}
			...
			if ( 'actions' === $column_name ) {
				$duplicate_link = add_query_arg(
					array(
						...
						'post'      => $post_id,
						'_wpnonce'  => wp_create_nonce( 'ywctm-duplicate-button_' . $post_id ),
					),
					admin_url( 'post.php' )
				);
				...
				$actions = array(
					...
					array(
						'type'   => 'action-button',
						'action' => 'duplicate',
						'title'  => esc_html__( 'Duplicate', 'yith-woocommerce-catalog-mode' ),
						'icon'   => 'clone',
						'url'    => $duplicate_link,
					),
					array(
		...
		public function set_bulk_actions( $actions ) {
			$actions = array(
				'delete' => __( 'Delete', 'yith-woocommerce-catalog-mode' ),
			);
		...
		public function admin_scripts() {
			...
			$embedded_fonts = ywctm_embedded_font_list();
			$theme_font     = ywctm_get_theme_font();
			$font_names     = array();

			foreach ( $embedded_fonts as $slug => $values ) {
				add_editor_style( YWCTM_ASSETS_URL . "fonts/$slug/style.css?display=swap" );
				wp_enqueue_style( "yith-gfont-$slug", YWCTM_ASSETS_URL . "fonts/$slug/style.css", array(), YWCTM_VERSION );
			}
			...
			if ( ! ywctm_is_multivendor_active() || ! ywctm_is_multivendor_integration_active() ) {
				$css = '.yith-plugin-fw-sub-tabs-nav > h3 > a { display: none !important }
					li.yith-plugin-fw-tab-element a.nav-tab i{ display: none !important }
		...
		public function tiny_mce_before_init( $settings, $editor_id ) {
			...
			if ( 'ywctm_label_text' === $editor_id ) {
				...
				/**
				 * APPLY_FILTERS: ywctm_button_editor_fonts
				 *
				 * Add or remove supported fonts from editor.
				 *
				 * @param array $fonts Supported fonts list.
				 *
				 * @return array
				 */
				$fonts = apply_filters(
				...
				/**
				 * APPLY_FILTERS: ywctm_button_font_sizes
				 *
				 * Add or remove supported font sizes.
				 *
				 * @param array $fonts Supported font sizes.
				 *
				 * @return array
				 */
		 ...
		 * @since   2.0.0
		 */
		public function add_default_fonts( $fonts ) {
			...
			foreach ( $embedded_fonts as $values ) {
				$name        = $values['name'];
				$fallback    = $values['fallback'];
				$font_family = "'$name', $fallback";
				$fonts[]     = "$name=$font_family";
			}
		...
		public function option_metabox( $post ) {
			...
			<div class="ywctm-metabox-topbar">
				<?php
				printf( '<a href="%1$s" class="ywctm-back-button" title="%2$s">%2$s</a>', esc_url( esc_url( add_query_arg( array( 'post_type' => $this->post_type ), admin_url( 'edit.php' ) ) ) ), esc_html__( '< Back to the Buttons & Labels table', 'yith-woocommerce-catalog-mode' ) );
				?>
			</div>
			<div class="woocommerce yith-plugin-fw-panel ywctm-metabox-wrapper">
				<div class="yith-plugin-ui yith-plugin-fw left-col">
					...
					<ul class="yith-plugin-fw__tabs">
						<?php foreach ( $groups as $key => $group ) : ?>
							<li class="yith-plugin-fw__tab">
								<a class="yith-plugin-fw__tab__handler yith-plugin-fw__tab__handler__<?php echo esc_attr( $key ); ?>" href="#yith-plugin-fw__tab__panel__<?php echo esc_attr( $key ); ?>"><?php echo esc_html( $group['label'] ); ?></a>
							</li>
						<?php endforeach; ?>
					</ul>
					<div class="ywctm-scrolling-content">
						<?php foreach ( $groups as $key => $group ) : ?>
							<div class="yith-plugin-fw__tab-panel yith-plugin-fw__tab-panel__<?php echo esc_attr( $key ); ?>" id="yith-plugin-fw__tab__panel__<?php echo esc_attr( $key ); ?>">
								<?php
				...
				<div class="ywctm-button-preview right-col">
					...
					<div class="ywctm-button-preview-container">
						<div class="ywctm-custom-button-container">
							...
							<span class="ywctm-inquiry-title">
								<?php printf( '<div class="btn-placeholder" style="text-align: center">%s</div>', esc_html_x( 'Contact Us', 'preview button placeholder text', 'yith-woocommerce-catalog-mode' ) ); ?>
							</span>
						</span>
						</div>
					</div>
		...
		public function save_post( $post_id, $post ) {
			...
			foreach ( $google_fonts as $name => $google_font ) {
				if ( preg_match( "/($name)+/mi", $posted['ywctm_label_text'] ) ) {
					$used_fonts[] = str_replace( ' ', '+', strtolower( $name ) );
				}
			}
		...
		public function duplicate_button() {
			...
			global $wpdb;
			$old_post = sanitize_text_field( wp_unslash( $_REQUEST['post'] ) );
		...
		protected function get_blank_state_params() {
			return array(
				'icon_url' => YWCTM_ASSETS_URL . '/icons/button-label.svg',
				'message'  => sprintf(
					// translators: all the placeholders are HTML tags to use in the string.
					esc_html_x(
						'%1$sYou don\'t have any button or label yet.%2$s%3$sCreate your first button or label now!%4$s',
				...
				'cta'      => array(
					'title' => esc_html__( 'Create a new button or label', 'yith-woocommerce-catalog-mode' ),
					'url'   => esc_url( add_query_arg( array( 'post_type' => $this->post_type ), admin_url( 'post-new.php' ) ) ),
		...
		/**
		 * Show epmty template if no button is created
		 *
		...
		public function maybe_render_blank_state( $which ) {
			...
			if ( $this->get_blank_state_params() && $this->is_empty_table() && $post_type === $this->post_type && 'bottom' === $which ) {
				$this->render_blank_state();

				echo '<style type="text/css">#posts-filter .wp-list-table, #posts-filter .yith-plugin-ui__wp-list-auto-h-scroll__wrapper, #posts-filter .tablenav.top, .tablenav.bottom > *, .wrap .subsubsub, .yith-plugin-fw-wp-page-wrapper.yith-plugin-ui--classic-wp-list-style .wrap .page-title-action { display: none; } #posts-filter .tablenav.bottom { height: auto; display: block } </style>';
			}
		}
		 ...
		 * @return bool
		 */
		protected function is_empty_table() {
			$posts = get_posts(
				array(
					'numberposts' => -1,
					'post_type'   => $this->post_type,
		...
		public function maybe_render_reset_filters_button( $which ) {
			...
			if ( $post_type === $this->post_type && 'top' === $which && $this->is_the_list_filtered() ) {
				$post_status = sanitize_text_field( wp_unslash( $_GET['post_status'] ?? 'all' ) ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended
				$args        = array_filter(
					array(
						'post_type'   => $this->post_type,
		...
		protected function is_the_list_filtered() {
			return ! empty( $_GET['m'] ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		}
	}
