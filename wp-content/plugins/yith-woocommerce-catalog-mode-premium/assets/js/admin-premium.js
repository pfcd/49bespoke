...
jQuery(
	function ( $ ) {
		...
		$( 'select[name^=ywctm_inquiry_form_type]' ).on(
			...
			function () {
				var reCAPTCHA            = $( '#ywctm_reCAPTCHA' ),
					default_form         = $( 'div.yith-plugin-fw__panel__option.yith-plugin-fw__panel__option--ywctm-default-form' ),
					default_form_section = default_form.closest( '.yith-plugin-fw__panel__section' );
		...
		// Handle deps for the fields in category/tag edit page.
		handle_deps();
		...
		var newExclusion = $( document ).find( '.ywctm-exclusion-list-popup-wrapper' ),
			...
			openPopup    = function ( action, id ) {
				...
				var title  = 'insert' === action ? ywctm.popup_labels.title : ywctm.popup_labels.edit,
					button = 'insert' === action ? ywctm.popup_labels.save : ywctm.popup_labels.edit;
				...
				if ( 'edit' === action ) {
					popupForm.find( '#item_type' ).prop( 'disabled', true );
					popupForm.find( 'tr.item_type' ).hide();
					popupForm.find( 'input[name="item_id"' ).val( id );
				}
						...
						buttons:     [ {
							...
							'click': function ( e ) {
								...
								var popup     = $( '.ywctm-exclusion-list-popup-wrapper' ),
									...
									element   = (is_vendor && 'insert' === action || ! is_vendor && 'edit' === action) ? popup.find( '#item_ids' ) : popup.find( '#' + item_type + '_ids' );

								if ( 'insert' === action && element.find( 'option' ).length === 0 ) {
									element
										.parent()
										.find( 'small' )
										.remove();
		...
		function load_popup( obj, action, id ) {
			var container = $( '.ywctm-exclusions' );
			...
			$.ajax(
				{
					...
					success: function ( response ) {
						if ( $( response ).find( '.ywctm-exclusion-list-popup-wrapper' ).length > 0 ) {
							$( '.ywctm-exclusion-list-popup-wrapper' ).html( '' ).html( $( response ).find( '.ywctm-exclusion-list-popup-wrapper' ).html() );
							openPopup( action, id );
						}
						container.unblock();
					}
		...
		$( document ).on(
			...
			function ( e ) {
				...
				load_popup( $( this ), 'edit', $( this ).parent().data( 'item_id' ) );
			}
		);
		...
		// Move button to create new item in the exclusion list near the page title.
		var add_exclusion_button     = $( '.page-title-action.yith-add-button' ),
			content                  = $( '#yith_wc_catalog_mode_panel_exclusions-items .yith-plugin-fw__panel__content__page .yith-plugin-fw__panel__content__page__title, #yith_wc_catalog_mode_panel_exclusions .yith-plugin-fw__panel__content__page .yith-plugin-fw__panel__content__page__title' ),
			vendor_exclusion_content = $( '#yith_wc_catalog_mode_panel_exclusions-vendors .yith-plugin-fw__panel__content__page .yith-plugin-fw__panel__content__page__title' );

		content.append( add_exclusion_button );
		vendor_exclusion_content.append( add_exclusion_button );

		$( '.ywctm-add-line-separator' ).prev( '.option-element' ).before( '<br/>' );

		// Add delete confirmation for the bulk actions in the exclusion list.
		$('#doaction').on('click', function(e) {
			...
			if ( 'delete' === action ) {
				...
				yith.ui.confirm(
					{
						title: ywctm.bulk_delete.confirm_title,
						message: ywctm.bulk_delete.confirm_message,
						confirmButtonType: 'delete',
						confirmButton: ywctm.bulk_delete.confirm_button,
						cancelButton: ywctm.bulk_delete.cancel_button,
