...
$sql_cate = "SELECT * FROM test_virtuemart_categories_en_gb WHERE slug IN ('100')";
$res_cate = $conn->query($sql_cate);
while($row_cate = $res_cate->fetch_assoc()){
	$category = get_term_by( 'slug', $row_cate['slug'], 'product_cat' );
	if(!empty($category->term_id)){
		$cat_id = $category->term_id;
	}
	
	$sql_prod = "SELECT * FROM test_virtuemart_products A JOIN test_virtuemart_products_en_gb B ON A.virtuemart_product_id=B.virtuemart_product_id WHERE A.virtuemart_product_id IN(SELECT virtuemart_product_id FROM test_virtuemart_product_categories WHERE virtuemart_category_id = '".$row_cate['virtuemart_category_id']."')";
